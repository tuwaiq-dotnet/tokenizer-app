	...
	public class NumberTokenizer : Tokenizable
	{
		...
		public override Token tokenize(Tokenizer t)
		{
			...
			if (isHexaStart(currentCharacter.ToString() + nextCharacter.ToString()))
			{
				...
				{
					if (isDot(currentCharacter))
						throw new Exception("Unexpected token");
					if (isExponent(currentCharacter) && readExponent)
						throw new Exception("Unexpected token");
					else if (isExponent(currentCharacter))
						readExponent = true;
					if (isSign(currentCharacter) && readSign)
						throw new Exception("Unexpected token");
					else if (isSign(currentCharacter))
						readSign = true;
					token.Value += t.input.step().Character;
					currentCharacter = t.input.peek();
				}
			...
			else if (isDecimalDigit(currentCharacter) || isDot(currentCharacter))
			{
				...
				{
					if (isDot(currentCharacter) && readDot)
						throw new Exception("Unexpected token");
					else if (isDot(currentCharacter))
						readDot = true;
					if (isExponent(currentCharacter) && readExponent)
						throw new Exception("Unexpected token");
					else if (isExponent(currentCharacter))
